...
_, winner_cards = game.play_game()
score = sum((i + 1) * card for i, card in enumerate(winner_cards))
print(score)
...
class RecursiveCombat(Combat):
    ...
    def play_game(self):
        ...
        return super(RecursiveCombat, self).play_game()

    def play_round(self):
        self.past_configurations.append((self.deck_p1.copy(), self.deck_p2.copy()))
        card_p1 = self.deck_p1.pop()
        card_p2 = self.deck_p2.pop()
        if (self.play_recurse_game(card_p1, card_p2)):
            ...
            winner, winner_cards = recurse_game.play_game()
        else:
    ...
    def repeated_configuration(self):
        return (self.deck_p1, self.deck_p2) in self.past_configurations
